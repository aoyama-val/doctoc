...
function addAnchor(mode, header) {
  ...
  // Delete some multibyte characters since they don't appear in the anchor generated by github.
  var DELETE_CHARS = /[：（）]/g;
  header.anchor = header.anchor.replace(/\(#.*\)/, function(all, g1) {
    var decoded = decodeURIComponent(all);
    var replaced = decoded.replace(DELETE_CHARS, "");
    return replaced;
  });
